...
vim.api.nvim_create_autocmd({"FileType"}, {
    pattern = {"php"},
    callback = function()
	vim.keymap.set({"n", "v"}, "<s-k>", [[:!searchPHPdocs "<cword>"<cr><cr>]])
    end,
})
vim.api.nvim_create_autocmd({"FileType"}, {
    pattern = {"py"},
    callback = function()
	vim.keymap.set("n", "<s-k>", [[:!searchPythonDocs.sh "<cword>"<cr><cr>]])
    end,
})
...
local function compose(tab1, key, val) local temp = tab1; temp[key] = val; return temp end
...
require('lspconfig')['clangd'].setup{
    on_attach = on_attach,
    flags = lsp_flags,
    capabilities = require('lspconfig')['clangd'].capabilities,
